...
| createUser | username, email, role | Initiate object called user with username, email and role, push new object to stored data. Write stored data to file. |
| editUser | userToEdit, updatedRole | iterate through all stored users, if users username is userToEdit, stored users role becomes updatedRole. |
| deleteUser | userToDelete | Iterate through stored users, if stored users username is userToDelete, remove user from stored users. Iterate through stored groups, if group members contain userToDelete, remove user. Iterate through stored channels, if channel members includes userToDelete, remove user. Write stored data to file |
| createGroup | groupName | Initiate newGroup object with groupName and username, push newGroup object to stored groups. Write stored data to file. |
| deleteGroup | groupName | Iterate through stored groups, if stored group name is groupName, remove it. |
